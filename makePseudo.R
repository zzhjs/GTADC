...
# num <- args[1]
# # 在R中处理输入值
# cat("Received input from Python:", num, "\n")
# filename<-paste('list_',num,'.txt', sep = "")
# sel.features <- read.table(filename, header = FALSE)[, 1]
#

#scRNA_data.csv:gene×cell
sc.count <- read.csv("./data/scRNA_data.csv", header=T, row.names = 1,check.names=FALSE)
# sc.count <- read.csv("./data/fileter_sc.csv", header=T, row.names = 1,check.names=FALSE)
...
metadata[,'type'][metadata[,'type']==""]="NAN"
...
test.spot.ls1<-test_spot_fun(se_obj=tem.t1,clust_vr='type',n=10000)
...
c1<- Seurat::CreateSeuratObject(counts = st_counts[[1]], min.cells = 0, min.features = 0)
c2<- Seurat::CreateSeuratObject(counts = st_counts[[2]], min.cells = 0, min.features = 0)
c1<-NormalizeData(c1)
c2<-NormalizeData(c2)
